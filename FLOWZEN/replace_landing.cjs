...
const landingPath = 'landing.html';
let content = fs.readFileSync(landingPath, 'utf8');

const sIdx = content.indexOf(`        loginForm.addEventListener('submit', (e) => {`);
const eIdx = content.lastIndexOf(`    </script>`);

if (sIdx !== -1 && eIdx !== -1) {
    const newScript = `        loginForm.addEventListener('submit', (e) => {
            e.preventDefault();
            handleLogin();
        });
...
`;
    ...
    fs.writeFileSync(landingPath, content.substring(0, sIdx) + newScript + '    </script>\n</body>\n</html>\n');
}
